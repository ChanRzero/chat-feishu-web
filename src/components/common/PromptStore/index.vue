...
import promptsList from '@/assets/prompts-zh.json'
...
const importLocalPrompt = (from = 'online') => {
  try {
    const jsonData = JSON.parse(tempPromptValue.value)
    let key = ''
    let value = ''
...
const impotLocalPromptTemplate = async () => {
  try {
    const jsonData = promptsList
...
const usePromptTemplate = (row: { key: string; value: string }) => {
  const systemMessage = row.value
  const temperature = ref(settingStore.temperature ?? 0.5)
  const top_p = ref(settingStore.top_p ?? 1)
  // 创建Partial<SettingsState>变量
  ...
  message.success(t('common.useSuccess'))
}
            ...
            <NListItem v-for="(item, index) of dataSource" :key="index">
              ...
              <template #suffix>
                <div class="flex flex-col items-center gap-2">
                  <NButton tertiary size="small" type="primary" @click="usePromptTemplate(item)">
                    {{ t('common.use') }}
                  </NButton>
